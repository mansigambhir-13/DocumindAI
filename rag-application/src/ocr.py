...
try:
    ...
    class Settings:
        PDF_DIRECTORY = str(project_root / "data" / "raw")  # Fixed path to data/raw
        PROCESSED_DOCUMENTS_DIR = str(project_root / "data" / "processed")  # Fixed path to data/processed
        DEBUG = True
        USE_OCR = True
...
class DocumentProcessor:
    def __init__(self):
        ...
        logger.info(f"Raw Documents Directory: {self.pdf_directory}")
        logger.info(f"Processed Output Directory: {self.output_dir}")
    ...
    def process_document_folder(self) -> List[Dict[str, Any]]:
        """
        Process all PDF and image documents in the data/raw folder
        
        Returns:
            List of processed document data
        """
        if not self.pdf_directory.exists():
            logger.error(f"Raw documents directory not found: {self.pdf_directory}")
            return []
...
def main():
    ...
    print(f"📁 Looking for 75 documents in: {project_root / 'data' / 'raw'}")
    ...
    if not processor.pdf_directory.exists():
        logger.error(f"Raw documents directory not found: {processor.pdf_directory}")
    ...
    if not all_files:
        ...
        print(f"💡 Please add your 75 PDF or image files to: {processor.pdf_directory}")
        return
    
    print(f"📁 Found {len(pdf_files)} PDF files and {len(image_files)} image files in {processor.pdf_directory}")
    print(f"🎯 Expected: 75 documents, Found: {len(all_files)} documents")
    ...
    if processed_docs:
        ...
        print(f"\n✅ Successfully processed {len(processed_docs)} out of {len(all_files)} documents")
        print(f"📄 Total pages: {total_pages}")
        print(f"📝 Total paragraphs: {total_paragraphs}")
        print(f"📁 Processed files saved to: {processor.output_dir}")
        print(f"📋 Processing summary saved to: {processor.output_dir}/processing_summary.json")
        
        # Check if we processed all 75 documents
        if len(all_files) == 75:
            print(f"🎯 Perfect! Found and processed all 75 documents as expected.")
        elif len(all_files) < 75:
            print(f"⚠️  Warning: Only found {len(all_files)} documents, expected 75.")
        else:
            print(f"ℹ️  Info: Found {len(all_files)} documents (more than expected 75).")
    else:
